...
pub async fn get_papers(
    ...
    let mut sql = "SELECT id, url, title, \
    ...
    #[cfg(feature = "ssr")]
    if !negative_query.is_empty() { // AI filtering is now handled progressively on the client
...
#[derive(
    Debug, Clone, Serialize, Deserialize, Default, PartialEq, bitcode::Encode, bitcode::Decode,
)]
...
pub async fn get_arxiv_config() -> Result<ArxivConfig, ServerFnError> {
...
pub async fn save_arxiv_config(
    ...
    let mut config: toml::Value = content
        .parse()
        .map_err(|e| ServerFnError::new(format!("Failed to parse TOML: {}", e)))?;

    if let Some(arxiv_val) = config.get_mut("arxiv") {
        if let Some(table) = arxiv_val.as_table_mut() {
            table.insert("category".to_string(), toml::Value::String(arxiv.category));

            table.insert(
                "start".to_string(),
                toml::Value::Integer(arxiv.start as i64),
            );

            table.insert(
                "max_results".to_string(),
                toml::Value::Integer(arxiv.max_results as i64),
            );

            table.insert(
                "lookback_days".to_string(),
                toml::Value::Integer(arxiv.lookback_days as i64),
            );
        }
    }
...
async fn verify_admin(password: &str) -> Result<bool, ServerFnError> {
    use argon2::Argon2;
    use argon2::password_hash::PasswordHash;
    use argon2::password_hash::PasswordVerifier;

    let assets_dir = if std::path::Path::new("assets").exists() {
        "assets"
    } else {
        "../assets"
    };

    let hash_path = std::path::Path::new(assets_dir).join("admin.bin");

    let hash_str = std::fs::read_to_string(hash_path).map_err(|e| {
        ServerFnError::new(format!(
            "Security system \
                 error: {}",
            e
...
pub async fn verify_admin_password(password: String) -> Result<bool, ServerFnError> {
...
pub async fn update_admin_password(
    ...
    if !verify_admin(&old_password).await? {
        return Err(ServerFnError::new(
            "Unauthorized: \
                 Invalid current \
                 password",
    ...
    let admin_bin_path = std::path::Path::new(assets_dir).join("admin.bin");

    // Remove existing file first to
    // avoid appending to stale data
    if admin_bin_path.exists() {
        std::fs::remove_file(&admin_bin_path)?;
    }

    std::fs::write(admin_bin_path, password_hash).map_err(|e| {
        ServerFnError::new(format!(
...
#[derive(Serialize, Deserialize, Encode, Decode, Clone, Debug, Default, PartialEq)]
...
pub static REPOSITORY: LazyLock<String> =
    LazyLock::new(|| "https://github.com/panayang/arxiv-daily".to_string());

#[allow(clippy::inline_always)]
#[inline(always)]
#[server(GetVersionInfo, "/api", input = Bitcode, output = Bitcode)]

pub async fn get_version_info() -> Result<VersionInfo, ServerFnError> {
    Ok(VersionInfo {
...
pub async fn fetch_new_articles(
    ...
    let url = if !end_date.is_empty() {
        ...
        let s_fmt = s.replace("-", "") + "0000";

        let e_fmt = end_date.replace("-", "") + "2359";
        ...
        let start = now - chrono::Duration::days(config.arxiv.lookback_days as i64);

        let s_fmt = start.format("%Y%m%d%H%M").to_string();

        let e_fmt = now.format("%Y%m%d%H%M").to_string();
    ...
    if !papers.is_empty() {
        ...
        query_builder.push_values(papers, |mut b, paper| {
            b.push_bind(paper.id)
                ...
                .push_bind(chrono::DateTime::from_timestamp(paper.updated, 0).unwrap_or_default())
                .push_bind(chrono::DateTime::from_timestamp(paper.published, 0).unwrap_or_default())
        ...
        query_builder.push( " ON CONFLICT(id) DO \
             UPDATE SET
            updated = excluded.updated, title = excluded.title, summary = excluded.summary, url = excluded.url", );
...
async fn ensure_schema(pool: &sqlx::SqlitePool) -> Result<(), ServerFnError> {
    ...
    .execute(pool)
    .await
    .map_err(|e| ServerFnError::new(format!("Table creation error: {}", e)))?;
...
static MODEL_GENERATION: std::sync::atomic::AtomicUsize = std::sync::atomic::AtomicUsize::new(0);

#[cfg(feature = "ssr")]

static MODEL: Mutex<Option<Arc<std::sync::Mutex<ai::Gemma3>>>> = Mutex::const_new(None);

#[cfg(feature = "ssr")]

static ACTIVE_MODEL_WEAK: Mutex<Option<std::sync::Weak<std::sync::Mutex<ai::Gemma3>>>> =
    Mutex::const_new(None);


#[server(UnloadModel, "/api/unload_model", input = Bitcode, output = Bitcode)]

pub async fn unload_model() -> Result<(), ServerFnError> {
    log::info!("Request to unload AI model...");
...
pub async fn stream_ai_filter(
    ...
    #[cfg(feature = "ssr")]
    {
        ...
        if negative_query.is_empty() {
            return Ok(ByteStream::new(futures::stream::iter::<
                Vec<Result<Bytes, ServerFnError>>,
            >(vec![])));
        }
        ...
        let (model_weak, current_gen) = {
            ...
            let r#gen = MODEL_GENERATION.load(Ordering::Relaxed);
            ...
            if let Some(m) = &*model_lock {
                ...
                loop {
                    ...
                    if let Some(weak) = &*weak_lock {
                        if weak.upgrade().is_some() {
                            if cleanup_retries > 10 {
                                // After ~2 seconds, give up
                                return Err(ServerFnError::new(
                                    "System busy: Previous model is still unloading. Please retry in a few seconds.",
                                ));
                            }

                            log::warn!(
                                "Old model still active (memory cleanup pending). Waiting..."
                            );
                ...
                if let Some(m) = &*model_lock {
                    ...
                    let (model_path, tokenizer_path) = if std::path::Path::new("assets/llm.bin")
                        .exists()
                    {
                    ...
                    *weak_lock = Some(Arc::downgrade(&m));
        ...
        let stream = futures::stream::iter(papers)
             .then(move |paper| { let model_weak = model_weak.clone();
                let negative_query = negative_query.clone();
                let params_gen = current_gen;

                async move { // 1. Check generation
                    let current = MODEL_GENERATION.load(Ordering::Relaxed);
                    if current != params_gen { log::info!("⏹️ Stream stopping: gen {} != {}", current, params_gen);
                         return Ok(FilterStreamMessage::Stopped);
                    }

                    // 2. Try to upgrade Weak
                    let model_arc = match model_weak.upgrade() { Some(arc) => arc, None => return Ok(FilterStreamMessage::Stopped), };

                    // 3. Yield to prevent CPU starvation on single-threaded runtimes or heavy load
                    tokio::time::sleep(tokio::time::Duration::from_millis(5)).await;

                    let summary_snippet = if paper.summary.len() > 300 { format!("{}...", &paper.summary[0..300])
                    } else { paper.summary.clone()
                    };
                    ...
                    let result = tokio::task::spawn_blocking(move || { let mut model = model_arc.lock().map_err(|_| "Poisoned lock")?;

                        // Check generation inside inference loop
                        // Use a non-move closure to ensure we read MODEL_GENERATION fresh each time
                        let check_cancel = || { let current = MODEL_GENERATION.load(Ordering::Relaxed);
                            let cancelled = current != params_gen;
                            if cancelled { log::info!("🛑 Cancellation detected: gen {} != {}", current, params_gen);
                            }
                            cancelled
                        };
                    ...
                    let response = match result { Ok(Ok(r)) => r, Ok(Err(e)) => { log::error!("AI completion internal error: {}", e);
                             return Ok(FilterStreamMessage::Result(paper.id, true));
                        }
                        Err(e) => { log::error!("AI task join error: {}", e);
                             return Ok(FilterStreamMessage::Result(paper.id, true));
                        }
                    };
            ...
            .map(|result| { match result { Ok(msg) => { let config = config::standard();
        ...
        return Ok(ByteStream::new(stream));
    }
...
fn fix_latex_symbols(latex: &str) -> String {
    latex
...
fn try_latex_to_mathml(
    ...
    let mut current_latex = latex.to_string();

    for _ in 0..15 {
        let result = latex_to_mathml(&current_latex, style);

        // Handle successful result but check if it contains embedded error messages
        if let Ok(ref mathml) = result {
            if !mathml.contains("PARSE ERROR") && !mathml.contains("Undefined") {
        ...
        if let Some(pos) = err_str.find("Command(\"") {
            let cmd_part = &err_str[pos + 9..];

            let end = cmd_part.find("\\\"").or_else(|| cmd_part.find("\")"));

            if let Some(end_idx) = end {
                let cmd = &cmd_part[..end_idx];

                if !cmd.is_empty() {
                    let to_replace = format!("\\{}", cmd);

                    let next = current_latex.replace(&to_replace, "");
        ...
        if err_str.contains("Circumflex") || err_str.contains("Circumflex") {
            let next = current_latex.replace('^', "");
        ...
        if err_str.contains("Underscore") {
            let next = current_latex.replace('_', "");
        ...
        if let Some(pos) = err_str.find("Undefined(\"") {
            let sym_part = &err_str[pos + 11..];

            if let Some(end) = sym_part.find("\")") {
                let sym = &sym_part[..end];

                if !sym.is_empty() {
                    let to_replace = format!("\\{}", sym);
        ...
        if let Some(pos) = current_latex.find('\\') {
            let after_slash = &current_latex[pos + 1..];
            ...
            if end > 0 {
                let to_remove = format!("\\{}", &after_slash[..end]);

                current_latex = current_latex.replace(&to_remove, "");
    ...
    format!("{}{}{}", delimiter, html_escape(latex), delimiter)
}
...
pub fn render_math(text: &str) -> String {
    ...
    static MATH_INLINE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?s)\$(.*?)\$").unwrap());
    ...
    // 3. Escape skeleton
    let mut final_result = html_escape(&result);

    // 4. Re-inject
    for (idx, rendered) in placeholders.into_iter().enumerate() {
        let placeholder = format!("__MATH_P_{}__", idx);

        final_result = final_result.replace(&placeholder, &rendered);
    }
...
fn filter_with_keywords(
    ...
    let keywords: Vec<String> = negative_query
        .split(',')
...
#[cfg(feature = "hydrate")]
#[leptos_workers::worker(HealthCheckWorker)]

pub async fn health_check_worker(req: HealthCheckRequest) -> HealthCheckResponse {
    let url = format!("{}/api/health", req.origin);

    let healthy = match gloo_net::http::Request::get(&url).send().await {
        | Ok(resp) => resp.ok(),
        | Err(_) => false,
    };

    HealthCheckResponse { is_healthy: healthy }
}
...
fn HealthBanner() -> impl IntoView {
    ...
    #[cfg(feature = "hydrate")]
    {
        ...
        spawn_local(async move {
            loop {
                ...
                match health_check_worker(HealthCheckRequest { origin }).await {
                    | Ok(resp) => {
                        set_backend_down.set(!resp.is_healthy);
                    },
                    | Err(_) => {
                        set_backend_down.set(true);
                    },
                }
    ...
    view! { <Show when=move || backend_down.get()>
            <div
                id="health-banner"
...
pub fn App() -> impl IntoView {
    ...
    view! { <Stylesheet id="leptos" href="/pkg/web_app.css"/>
...
fn Dashboard() -> impl IntoView {
    ...
    let (negative_query, set_negative_query) = signal("".to_string());

    let (show_fetch_status, set_show_fetch_status) = signal(false);
    ...
    let (show_surprise, set_show_surprise) = signal(false);

    let (hidden_paper_ids, set_hidden_paper_ids) = signal(std::collections::HashSet::new());

    let (view_mode, set_view_mode) = signal("card".to_string());
    ...
    let page_size = RwSignal::new(51usize);
    ...
    let total_count = Resource::new(
        ...
        |(query, category, date, end_date)| {
            async move { get_paper_count(query, category, date, end_date).await }
        },
    );
    ...
    Effect::new(move |_| {
        let neg_query = negative_query.get();
    ...
    Effect::new(move |_| {
        let papers_result = papers.get();

        let neg_query = negative_query.get();

        let use_llm_flag = use_llm.get();

        if let Some(Ok(current_papers)) = papers_result {
            // Only stream if we have papers, a negative query, and LLM is enabled
            if !neg_query.is_empty() && use_llm_flag {
                spawn_local(async move {
                    // Start fresh
                            ...
                            while let Some(chunk_res) = stream.next().await {
                                match chunk_res {
                                    | Ok(chunk) => {
                                        ...
                                        if let Ok((msg, _size)) = bincode_next::decode_from_slice::<
                                            FilterStreamMessage,
                                            _,
                                        >(
                                            &chunk, config
    ...
    let unload_action = Action::new(|_| async move { unload_model().await });
    ...
    Effect::new(move |_| {
        let pending = fetch_action.pending().get();

        let value = fetch_action.value().get();

        if pending {
            set_show_fetch_status.set(true);
    ...
    let on_reset = move |_| {
        ...
        set_negative_query.set("".to_string());
    ...
    #[cfg(feature = "hydrate")]
    {
        ...
        let total_count_resource = total_count;
        ...
        window_event_listener(ev::keydown, move |ev| {
            ...
            if !show_config.get() && !show_about.get() && !show_surprise.get() {
                ...
                if active_el != "INPUT" && active_el != "TEXTAREA" && active_el != "SELECT" {
                    if key == "ArrowLeft" {
                        ...
                        let current = page.get();
                        let total = total_count_resource
                            .get()
                            .and_then(|res| res.ok())
                            .unwrap_or(0);
    ...
    let on_page_change = move |p: usize| {
        ...
        // #[cfg(feature = "hydrate")]
        // { // if let Some(window) = web_sys::window() { // window.scroll_to_with_x_and_y(0.0, 400.0);
        // }
        // }
    };

    view! { <div class="max-w-7xl mx-auto px-4 py-8 md:px-8 md:py-12 space-y-12">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex gap-2 animate-slide-up stagger-1">
                        <button
                            ...
                            class=move || format!( "p-3 rounded-xl transition-all border {}", if view_mode.get() == "card" { "bg-obsidian-accent/20 border-obsidian-accent/30 text-obsidian-accent"
                                } else { "bg-obsidian-sidebar/50 border-white/5 text-obsidian-text/40 hover:text-obsidian-text hover:border-white/10"
                                }
                            )
                            title="Card View"
                        >
                        ...
                        <button
                            ...
                            class=move || format!( "p-3 rounded-xl transition-all border {}", if view_mode.get() == "list" { "bg-obsidian-accent/20 border-obsidian-accent/30 text-obsidian-accent"
                                } else { "bg-obsidian-sidebar/50 border-white/5 text-obsidian-text/40 hover:text-obsidian-text hover:border-white/10"
                                }
                            )
                            title="List View"
            ...
            {move || { let pending = fetch_action.pending();
                let value = fetch_action.value();

                view! { <Show when=move || show_fetch_status.get()>
                        <div class="fixed top-8 left-1/2 -translate-x-1/2 z-[100] animate-slide-up">
                            <div class="glass border border-white/10 rounded-2xl px-6 py-3 shadow-2xl flex items-center gap-4">
                                <Show
                                    ...
                                    fallback=move || { value.with(|v| { match v { Some(Ok(count)) => { let count = *count;
                                                    view! { <div class="flex items-center gap-3">
                                                ...
                                                Some(Err(e)) => view! { <div class="flex items-center gap-3">
            ...
            <FilterBar
                ...
                set_date_filter
                on_fetch=Callback::new(move |_| { fetch_action.dispatch((selected_category.get(), date_filter.get(), end_date_filter.get()));
                })
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8">
                <Transition fallback=move || view! { <div class="col-span-full py-48 flex flex-col items-center justify-center space-y-6 animate-fade-in">
                        <div class="relative">
                    ...
                    {move || { match papers.get() { Some(Ok(current_papers)) => { if current_papers.is_empty() { view! { <div class="col-span-full py-32 text-center space-y-4">
                            ...
                            Some(Err(e)) => { view! { <div class="col-span-full py-20 bg-red-500/10 border border-red-500/20 rounded-2xl p-8 text-center space-y-4">
...
fn PaperCard(paper: Paper) -> impl IntoView {
    ...
    view! { <div class="glass glow-hover rounded-3xl p-8 flex flex-col h-full active:scale-[0.98] animate-slide-up group relative overflow-hidden">
            ...
            <div class="relative flex-grow z-10">
                <div
                    ...
                    inner_html={ let value = summary.clone();
                        move || render_math(&value)
                    }
                ></div>
                {move || if !expanded.get() && summary.len() > 180 { view! { <button
            ...
            <div class="mt-8 flex items-center gap-4 relative z-10">
                {if let Some(link) = pdf_link { view! { <a
                            href=link
                            target="_blank"
                ...
                } else { view! { <div class="flex-1 py-3.5 text-center text-[10px] text-obsidian-text/20 uppercase tracking-[0.2em] font-black border border-white/5 rounded-2xl">"Paper Locked"</div>
                    }.into_any()
                }}
...
fn ConfigModal(
    ...
    fetch_action: Action<(String, String, String), Result<usize, ServerFnError>>,
    selected_category: Signal<String>,
) -> impl IntoView {
    let (is_admin, set_is_admin) = signal(false);

    let (admin_password, set_admin_password) = signal("".to_string());

    let (auth_error, set_auth_error) = signal(None::<String>);

    let (show_password_change, set_show_password_change) = signal(false);
    ...
    let (arxiv_cat, set_arxiv_cat) = signal("".to_string());

    let (arxiv_start, set_arxiv_start) = signal(0);

    let (arxiv_max, set_arxiv_max) = signal(50);

    let (arxiv_lookback, set_arxiv_lookback) = signal(7);

    let (sync_start, set_sync_start) = signal("".to_string());

    let (sync_end, set_sync_end) = signal(chrono::Utc::now().format("%Y-%m-%d").to_string());

    Effect::new(move |_| {
        if let Some(Ok(c)) = arxiv_resource.get() {
            set_arxiv_cat.set(c.category);

            set_arxiv_start.set(c.start);

            set_arxiv_max.set(c.max_results);

            set_arxiv_lookback.set(c.lookback_days);
        }
    });

    let (manual_cat_override, set_manual_cat_override) = signal(None::<String>);

    let save_arxiv_action = Action::new(|input: &(String, ArxivConfig)| {
        let password = input.0.clone();

        let arxiv = input.1.clone();

        async move { save_arxiv_config(password, arxiv).await }
    });

    let (local_fetching, set_local_fetching) = signal(false);
        ...
        #[cfg(feature = "hydrate")]
        {
            if let Some(window) = web_sys::window() {
                if let Ok(Some(storage)) = window.local_storage() {
                    if let Ok(Some(val)) = storage.get("local_fetching") {
                        set_local_fetching.set(val == "true");
                    }
                }
            }
        }
    ...
    let save_config_action = Action::new(|input: &(String, String)| {
        let password = input.0.clone();

        let content = input.1.clone();

        async move { save_config(password, content).await }
    });

    let update_password_action = Action::new(|input: &(String, String)| {
        let old = input.0.clone();

        let new = input.1.clone();

        async move { update_admin_password(old, new).await }
    });

    let (config_content, set_config_content) = signal("".to_string());
    ...
    let verify_admin_action = Action::new(|password: &String| {
        let p = password.clone();

        async move { verify_admin_password(p).await }
    });

    Effect::new(move |_| {
        if let Some(Ok(true)) = verify_admin_action.value().get() {
            set_is_admin.set(true);

            set_auth_error.set(None);
        } else if let Some(Ok(false)) = verify_admin_action.value().get() {
                ...
                <div class="glass-dark border border-white/10 rounded-[2.5rem] w-full max-w-2xl shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden animate-scale-in">
                    ...
                    <div class="p-8 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        <Show
                            ...
                            fallback=move || view! { <div class="space-y-8 animate-fade-in">
                                    ...
                                    <div class="space-y-6">
                                        <div class="flex items-center justify-between p-6 bg-white/5 rounded-3xl border border-white/5 hover:border-obsidian-accent/20 transition-all group">
                                            ...
                                            <button
                                                ...
                                                class=move || format!( "w-12 h-6 rounded-full transition-all flex items-center px-1 {}", if use_llm.get() { "bg-obsidian-accent" } else { "bg-white/10" }
                                                )
                                            >
                                                <div class=move || format!( "w-4 h-4 bg-white rounded-full shadow-lg transition-all transform {}", if use_llm.get() { "translate-x-6" } else { "translate-x-0" }
                                                ) />
                                            </button>
                                        </div>
                                        ...
                                        <div class="flex items-center justify-between p-6 bg-white/5 rounded-3xl border border-white/5 hover:border-obsidian-accent/20 transition-all group">
                                            ...
                                            <button
                                                on:click=move |_| { let new_val = !local_fetching.get();
                                                    set_local_fetching.set(new_val);
                                                    #[cfg(feature = "hydrate")]
                                                    { if let Some(window) = web_sys::window() { if let Ok(Some(storage)) = window.local_storage() { let _ = storage.set("local_fetching", if new_val { "true" } else { "false" });
                                                            }
                                                        }
                                                    }
                                                }
                                                class=move || format!( "w-12 h-6 rounded-full transition-all flex items-center px-1 {}", if local_fetching.get() { "bg-obsidian-accent" } else { "bg-white/10" }
                                                )
                                            >
                                                <div class=move || format!( "w-4 h-4 bg-white rounded-full shadow-lg transition-all transform {}", if local_fetching.get() { "translate-x-6" } else { "translate-x-0" }
                                                ) />
                                            </button>
                                        </div>
                                        ...
                                        <div class="p-6 bg-white/5 rounded-3xl border border-white/5 space-y-4">
                                            ...
                                            <div class="flex items-center gap-3 px-2 mb-4">
                                                <button
                                                    on:click=move |_| { let now = chrono::Utc::now();
                                            ...
                                            <button
                                                on:click=move |_| { fetch_action.dispatch(( selected_category.get(), sync_start.get(), sync_end.get()
                                                    ));
                                                }
                                            ...
                                            >
                                                {move || if fetch_action.pending().get() { view! { <div class="w-3 h-3 border-2 border-obsidian-accent/20 border-t-obsidian-accent rounded-full animate-spin"></div> }.into_any()
                                                } else { view! { <svg class="w-3.5 h-3.5 group-hover:rotate-180 transition-transform duration-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div class="p-6 bg-white/5 rounded-3xl border border-white/5 space-y-4">
                                            ...
                                            <input
                                                ...
                                                on:input=move |ev| { if let Ok(val) = event_target_value(&ev).parse::<usize>() { page_size.set(val);
                                                    }
                                                }
                                    ...
                                    <div class="pt-8 border-t border-white/5 flex flex-col items-center gap-4">
                                        ...
                                        <div class="flex w-full gap-2">
                                            <input
                                                ...
                                                on:keydown=move |ev| { if ev.key() == "Enter" { verify_admin_action.dispatch(admin_password.get());
                                                    }
                                                }
                        ...
                        >
                            ...
                            <div class="space-y-8 animate-fade-in">
                                <Show
                                    ...
                                    fallback=move || view! { <div class="space-y-6">
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <button
                                                    on:click=move |_| { if new_password.get() == confirm_password.get() && !new_password.get().is_empty() { update_password_action.dispatch((admin_password.get(), new_password.get()));
                                    ...
                                    <div class="space-y-8 animate-fade-in">
                                        ...
                                        <div class="space-y-6 bg-white/5 p-6 rounded-[2rem] border border-white/5">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                ...
                                                <div class="space-y-2">
                                                    ...
                                                    <input
                                                        type="number"
                                                        class="w-full bg-black/20 border border-white/5 rounded-2xl px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-obsidian-accent/20 transition-all font-mono"
                                                        on:input=move |ev| { if let Ok(val) = event_target_value(&ev).parse::<i32>() { set_arxiv_start.set(val);
                                                            }
                                                        }
                                                ...
                                                <div class="space-y-2">
                                                    ...
                                                    <input
                                                        type="number"
                                                        class="w-full bg-black/20 border border-white/5 rounded-2xl px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-obsidian-accent/20 transition-all font-mono"
                                                        on:input=move |ev| { if let Ok(val) = event_target_value(&ev).parse::<i32>() { set_arxiv_max.set(val);
                                                            }
                                                        }
                                                ...
                                                <div class="space-y-2">
                                                    ...
                                                    <div class="flex items-center gap-3">
                                                        <input
                                                            ...
                                                            on:input=move |ev| { if let Ok(val) = event_target_value(&ev).parse::<i32>() { set_arxiv_lookback.set(val);
                                                                }
                                                            }
                                            ...
                                            <div class="flex justify-end">
                                                <button
                                                    on:click=move |_| { save_arxiv_action.dispatch((admin_password.get(), ArxivConfig { category: arxiv_cat.get(), start: arxiv_start.get(), max_results: arxiv_max.get(), lookback_days: arxiv_lookback.get(), }));
                                                    }
                                        ...
                                        <div class="flex justify-end gap-3 pt-4">
                                            ...
                                            <button
                                                on:click=move |_| { save_config_action.dispatch((admin_password.get(), config_content.get()));
                                                }
...
fn Pagination(
    ...
    view! { <div class="flex flex-col items-center gap-6 pt-12 border-t border-white/5">
            <div class="flex items-center gap-4">
                <button
                ...
                <div class="flex items-center gap-4 px-6 py-2 glass rounded-2xl shadow-inner">
                    ...
                    <input
                        ...
                        on:input=move |ev| { let val = event_target_value(&ev);
                            if let Ok(n) = val.parse::<usize>()
                                && n >= 1 && n <= total_pages.get() { on_page_change.run(n);
                                }
                            }
                        on:keydown=move |ev| { if ev.key() == "Enter" { let val = event_target_value(&ev);
                                if let Ok(n) = val.parse::<usize>()
                                    && n >= 1 && n <= total_pages.get() { on_page_change.run(n);
                                    }
...
fn FilterBar(
    ...
    set_negative_query: WriteSignal<String>,
    on_fetch: Callback<()>,
    ...
    view! { <div class="glass flex flex-wrap items-center gap-6 p-6 rounded-[2rem] border border-white/5 shadow-2xl animate-fade-in stagger-2 relative z-50">
                        ...
                        <div class="absolute z-[999] mt-3 w-full bg-obsidian-card border border-white/10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.8)] rounded-[1.5rem] overflow-hidden animate-scale-in">
                            <div class="p-2 border-b border-white/5">
                                <input
                                    ...
                                    on:keydown=move |ev| { if ev.key() == "Enter" { on_search.run(());
                            ...
                            <div class="max-h-60 overflow-y-auto custom-scrollbar">
                                <For
                                    ...
                                    children=move |(code, name)| { let is_active = move || selected_category.get() == code;
                                        view! { <button
                                                on:click=move |_| { set_selected_category.set(code.to_string());
                                                    set_is_open.set(false);
                                                    set_category_search.set("".to_string());
                                                }
                                                class=move || format!( "w-full text-left px-4 py-2.5 text-xs transition-colors hover:bg-obsidian-accent/10 flex items-center justify-between {}", if is_active() { "text-obsidian-accent bg-obsidian-accent/5 font-bold" } else { "text-obsidian-text/70" }
                                                )
                                            >
                                ...
                                />
                                {move || if filtered_categories.get().is_empty() { view! { <div class="px-4 py-3 text-xs text-obsidian-text/30 italic text-center">"No categories found"</div> }.into_any()
                                } else { ().into_any()
                                }}
                            </div>
                        </div>
            ...
            <div class="flex flex-col gap-2 min-w-[200px]">
                ...
                <div class="flex gap-2">
                    <input
                        ...
                        on:input=move |ev| { if let Ok(n) = event_target_value(&ev).parse::<usize>() { if n > 0 { set_page_size.set(n);
                                }
                            }
                        }
                        on:blur=move |_| { // Validate and adjust based on view mode when user finishes typing
                            let current = page_size.get();
                            if view_mode.get() == "card" && current % 3 != 0 { // Round to nearest multiple of 3
                                let adjusted = ((current + 1) / 3) * 3;
                                if adjusted > 0 { set_page_size.set(adjusted);
                                }
                            }
                        }
                        prop:value=move || page_size.get().to_string()
                    ...
                    <div class="relative group/presets">
                        ...
                        <div class="hidden group-hover/presets:block absolute right-0 mt-2 bg-obsidian-card border border-white/10 rounded-2xl shadow-2xl p-2 z-[100] min-w-[140px]">
                            ...
                            {move || { let presets = if view_mode.get() == "card" { vec![12, 24, 51, 99, 201]
                                } else { vec![10, 25, 50, 100, 200]
                                };
                                presets.into_iter().map(|size| { view! { <button
            ...
            <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto self-end justify-center lg:justify-end">
                ...
                >
                    <Show
                        ...
                        fallback=move || view! { <svg class="w-4 h-4 text-obsidian-accent group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
fn BackToTop() -> impl IntoView {
    ...
    Effect::new(move |_| {
        log::info!("BackToTop visibility: {}", show.get());
    });
    ...
    view! { <div class="fixed bottom-12 right-12 z-[9999]" style="pointer-events: none;">
                    ...
                    <div class="p-8 space-y-8 overflow-y-auto flex-1 custom-scrollbar">
                        ...
                        <div class="space-y-4">
                            <Transition fallback=move || view! { <div class="p-8 text-center animate-pulse text-obsidian-text/10 text-xs font-black uppercase tracking-widest">"Accessing Build Records..."</div> }>
                                {move || version_resource.get().map(|res| { match res { Ok(info) => view! { <div class="space-y-6">
                                        ...
                                        }.into_any(), Err(e) => view! { <div class="p-4 text-red-400 text-xs font-mono bg-red-500/5 rounded-lg border border-red-500/10">"Error: "{e.to_string()}</div> }.into_any(), }
                                })}
                            </Transition>
                        </div>
...
fn PaperListItem(paper: Paper) -> impl IntoView {
    let (expanded, set_expanded) = signal(false);
    ...
    view! { <div class="glass glow-hover rounded-2xl p-6 flex items-start gap-6 hover:scale-[1.01] active:scale-[0.99] transition-all duration-300 group relative overflow-hidden">
            ...
            <div class="flex-1 space-y-4 relative z-10">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1">
                        ...
                        <div class="space-y-2">
                            <p class=move || if expanded.get() { "text-sm text-obsidian-text/60 leading-relaxed font-medium"
                            } else { "text-sm text-obsidian-text/60 leading-relaxed line-clamp-2 font-medium"
                            }>
                                {summary.clone()}
                            </p>
