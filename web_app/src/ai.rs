...
impl RmsNorm {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let x_normed = x.to_dtype(internal_dtype)?.broadcast_div(&(norm_x + self.eps)?.sqrt()?)?;
        x_normed
            .to_dtype(x_dtype)?
...
impl RotaryEmbedding {
    fn new(dtype: DType, cfg: &Config, base: f64, dev: &Device) -> Result<Self> {
...
impl MLP {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        (lhs * rhs)?.apply(&self.down_proj.inner)
    }
}
...
#[cfg(feature = "ssr")]
impl Attention {
    fn forward(&mut self, xs: &Tensor, mask: Option<&Tensor>, seqlen_offset: usize) -> Result<Tensor> {
        let (b_sz, q_len, _) = xs.dims3()?;
        let q = self.q_proj.forward(xs)?;
        let k = self.k_proj.forward(xs)?;
        let v = self.v_proj.forward(xs)?;

        let q = q.reshape((b_sz, q_len, self.num_heads, self.head_dim))?.transpose(1, 2)?;
        let k = k.reshape((b_sz, q_len, self.num_kv_heads, self.head_dim))?.transpose(1, 2)?;
        let v = v.reshape((b_sz, q_len, self.num_kv_heads, self.head_dim))?.transpose(1, 2)?;
        ...
        out.transpose(1, 2)?.reshape((b_sz, q_len, ()))?.apply(&self.o_proj.inner)
    }
}

#[cfg(feature = "ssr")]
fn repeat_kv(xs: &Tensor, n: usize) -> Result<Tensor> {
    if n == 1 {
        Ok(xs.clone())
    } else {
        let (b_sz, n_heads, seq_len, head_dim) = xs.dims4()?;
        xs.unsqueeze(2)?
            .expand((b_sz, n_heads, n, seq_len, head_dim))?
            .reshape((b_sz, n_heads * n, seq_len, head_dim))
    }
}
...
impl Model {
    pub fn from_gguf<P: AsRef<Path>>(path: P, device: &Device) -> anyhow::Result<Self> {
        ...
        let embed_tokens_weight = content.tensor(&mut file, "token_embd.weight", device)?.dequantize(device)?;
        ...
        let rotary_emb_global = Arc::new(RotaryEmbedding::new(DType::F32, &cfg, 1_000_000.0, device)?);
        ...
        for i in 0..cfg.num_hidden_layers {
            ...
            let rotary_emb = if is_global { rotary_emb_global.clone() } else { rotary_emb_local.clone() };
    ...
    pub fn forward(&mut self, input_ids: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        ...
        let mask = if seq_len <= 1 {
            ...
            let mask = mask.unsqueeze(0)?.unsqueeze(0)?.to_dtype(self.dtype)?;
            Some(mask)
        };
        ...
        let logits = xs.apply(&self.lm_head.inner)?;
...
impl Config {
    fn num_heads(&self) -> usize { self.num_attention_heads }
    fn num_kv_heads(&self) -> usize { self.num_key_value_heads }
}
...
impl Gemma3 {
    pub fn new<P: AsRef<Path>>(model_path: P, tokenizer_path: P) -> anyhow::Result<Self> {
        ...
        Ok(Self { model, tokenizer, device, initialized: false })
    }
    ...
    pub fn complete(&mut self, prompt: &str, max_tokens: usize) -> anyhow::Result<String> {
        ...
        for i in 0..max_tokens {
            ...
            let next_token = logits_f32.argmax(D::Minus1)?.to_scalar::<u32>()?;
            let max_logit = logits_f32.max(D::Minus1)?.to_scalar::<f32>()?;
            ...
            let token_text = self.tokenizer.decode(&[next_token], true).map_err(anyhow::Error::msg)?;
            generated.push_str(&token_text);
            
            if token_text.contains("<end_of_turn>") || token_text.contains("<|im_end|>") || next_token == 1 || next_token == 107 {
                break;
            }
        }
