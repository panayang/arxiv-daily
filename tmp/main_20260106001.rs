use chrono::{DateTime, Utc};
use feed_rs::model::Entry;
use serde::Serialize;
use sqlx::sqlite::SqlitePool;
use std::time::Duration;

#[derive(Debug, Serialize)]
struct Paper {
    id: String,
    title: String,
    updated: DateTime<Utc>,
    published: DateTime<Utc>,
    summary: String,
    primary_category: String,
    categories: String, // Stored as comma-separated
    authors: String,    // Stored as JSON
    pdf_link: Option<String>,
}

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // 1. Initialize Database
    let pool = SqlitePool::connect("sqlite:arxiv_daily.db?mode=rwc").await?;
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS papers (
            id TEXT PRIMARY KEY, title TEXT, updated DATETIME, 
            published DATETIME, summary TEXT, primary_category TEXT, 
            categories TEXT, authors TEXT, pdf_link TEXT
        )",
    )
    .execute(&pool)
    .await?;

    // 2. Fetch from arXiv (hep-th)
    // arXiv supports max_results=2000 per call.
    println!("Fetching from arXiv...");
    let url = "http://export.arxiv.org/api/query?search_query=cat:hep-th&start=0&max_results=2000&sortBy=submittedDate&sortOrder=descending";
    let client = reqwest::Client::new();
    let response = client.get(url).send().await?.bytes().await?;

    // 3. Parse and Transform
    let feed = feed_rs::parser::parse(&response[..])?;
    println!("Found {} entries. Processing...", feed.entries.len());

    // Use Rayon for CPU-bound transformation
    let papers: Vec<Paper> = tokio::task::spawn_blocking(move || {
        use rayon::prelude::*;
        feed.entries.into_par_iter().map(transform_entry).collect()
    })
    .await?;

    // 4. Batch Save to SQLite
    println!("Saving {} papers to database...", papers.len());

    // Start one transaction for all 2000 papers
    let mut tx = pool.begin().await?;

    for paper in papers {
        sqlx::query(
            "INSERT INTO papers (id, title, updated, published, summary, primary_category, categories, authors, pdf_link)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET
        updated = excluded.updated,
        title = excluded.title,
        summary = excluded.summary"
        )
        .bind(&paper.id)
        .bind(&paper.title)
        .bind(paper.updated)
        .bind(paper.published)
        .bind(&paper.summary)
        .bind(&paper.primary_category)
        .bind(&paper.categories)
        .bind(&paper.authors)
        .bind(&paper.pdf_link)
        .execute(&mut *tx) // Execute inside the transaction
        .await?;
    }

    // Commit tells SQLite to actually save to the .db file
    tx.commit().await?;

    println!("Success! All papers saved.");

    println!("Done! Check arxiv_daily.db");
    Ok(())
}

fn transform_entry(entry: Entry) -> Paper {
    let authors: Vec<String> = entry.authors.iter().map(|a| a.name.clone()).collect();

    // Extract PDF link specifically
    let pdf_link = entry
        .links
        .iter()
        .find(|l| l.media_type.as_deref() == Some("application/pdf"))
        .map(|l| l.href.clone());

    Paper {
        id: entry.id,
        title: entry.title.map(|t| t.content).unwrap_or_default(),
        updated: entry.updated.unwrap_or(Utc::now()),
        published: entry.published.unwrap_or(Utc::now()),
        summary: entry.summary.map(|s| s.content).unwrap_or_default(),
        primary_category: entry
            .categories
            .first()
            .map(|c| c.term.clone())
            .unwrap_or_default(),
        categories: entry
            .categories
            .iter()
            .map(|c| c.term.clone())
            .collect::<Vec<_>>()
            .join(","),
        authors: serde_json::to_string(&authors).unwrap_or_default(),
        pdf_link,
    }
}
